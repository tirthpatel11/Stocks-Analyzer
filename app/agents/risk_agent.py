"""
Risk & Portfolio Construction Agent — "Guardrails"

This agent is responsible for:
- Position sizing and risk management
- Portfolio construction and optimization
- Risk metrics calculation (VaR, drawdown, etc.)
- Trade guardrails and limits
...
class RiskAgent(BaseAgent):
    ...
    def __init__(self):
        super().__init__(
            ...
            temperature=0.0  # Conservative for risk calculations
        )
...
- Provide final risk-adjusted recommendations to the Supervisor"""
    ...
    def calculate_position_size(
        ...
        Args:
            ...
            risk_per_trade_pct: Maximum % of account to risk per trade
            stop_loss_pct: Fixed stop loss percentage (if None, uses ATR)
        ...
        try:
            ...
            # Target 1% daily portfolio volatility from this position
            target_position_volatility = 0.01  # 1% daily
            volatility_adjusted_allocation = target_position_volatility / daily_volatility
            position_value_vol_adjusted = account_size * min(volatility_adjusted_allocation, 0.25)  # Max 25%
            shares_vol_adjusted = int(position_value_vol_adjusted / current_price)
            
            # Method 3: Kelly Criterion (simplified)
            # Estimate win rate and win/loss ratio from historical data
            ...
            # Recommended position (most conservative of the three)
    ...
    def compute_var(
        ...
        try:
            ...
            for conf in confidence_levels:
                ...
                # Method 3: Modified VaR (Cornish-Fisher adjustment)
    ...
    def analyze_drawdown(ticker: str, period: str = "5y") -> Dict[str, Any]:
        ...
        try:
            ...
            # Time underwater (% of time in drawdown)
            underwater_pct = (drawdown < 0).sum() / len(drawdown) * 100
            
            # Ulcer Index (measure of drawdown pain)
            ...
            return {
                ...
                "significant_drawdowns": significant_dds[-5:],  # Last 5
                "current_status": {
    ...
    def generate_risk_limits(
        ...
        """
        Generate risk limits and guardrails for trading
        ...
        Returns:
            Dictionary with risk limits and guardrails
        """
        try:
            stock = yf.Ticker(ticker)
            ...
            target_position_vol = 0.02  # 2% daily vol contribution
    ...
    async def execute(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute the Risk Agent's analysis
...
- Position Value: ₹{position_sizing.get('position_sizing', {}).get('recommended', {}).get('value', 'N/A'):,.2f}
- % of Account: {position_sizing.get('position_sizing', {}).get('recommended', {}).get('pct_of_account', 'N/A')}%

VALUE AT RISK (95% confidence, 1-day):
- Historical VaR: ₹{var_analysis.get('value_at_risk', {}).get('95%', {}).get('historical', {}).get('var_dollar', 'N/A'):,.2f}
- Modified VaR: ₹{var_analysis.get('value_at_risk', {}).get('95%', {}).get('modified', {}).get('var_dollar', 'N/A'):,.2f}
...
"""
            ...
            # Include analysis from other agents if available
